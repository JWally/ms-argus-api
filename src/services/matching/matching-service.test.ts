...
describe('MatchingService', () => {
  ...
  describe('tier05CookieLookup', () => {
    it('should return null when evercookie not found', async () => {
      ...
      const result = await service.tier05CookieLookup('tenant1', 'unknown-cookie');
      expect(result).toBeNull();
    });
    ...
    it('should match on fuzzy_hash with 0.85 confidence when stable_hash not found', async () => {
      dynamoMock
        .on(GetItemCommand, {
          ...
          Key: { tenant_id: { S: 'tenant1' }, hash_key: { S: 'stable#stable123' } },
        })
        .resolves({ Item: undefined })
        .on(GetItemCommand, {
          TableName: testConfig.tier1IndexTable,
          Key: { tenant_id: { S: 'tenant1' }, hash_key: { S: 'fuzzy#fuzzy456' } },
        })
        .resolves({
          Item: marshall({
            tenant_id: 'tenant1',
  ...
  describe('buildBucketKeys', () => {
    ...
    it('should build ip_ja4 bucket key', () => {
      ...
      const keys = service.buildBucketKeys('tenant1', fingerprint);
      expect(keys).toContain('tenant1#ip_ja4#192.168.1.1#t13d1516h2_8daaf6152771_02713d6af862');
    });
  ...
  describe('tier2CompoundMatch', () => {
    ...
    it('should return match when device appears in 2+ buckets', async () => {
      // Mock all three bucket lookups to return the same device
      dynamoMock
        .on(GetItemCommand, { TableName: testConfig.tier2BucketsTable })
        .resolves({
          Item: marshall({
            bucket_key: 'bucket1',
            device_ids: ['dev_match', 'dev_other'],
          }),
        });
  ...
  describe('tier2CompoundMatchWithTimeout', () => {
    it('should return null if matching takes too long', async () => {
      // Make DynamoDB calls slow
      dynamoMock.on(GetItemCommand).callsFake(async () => {
      ...
      const result = await service.tier2CompoundMatchWithTimeout('tenant1', fingerprint);
      expect(result).toBeNull();
    });
  });
  ...
  describe('runTieredMatching', () => {
    ...
    it('should fall through to Tier 1 when evercookie not found', async () => {
      dynamoMock
        .on(GetItemCommand, {
          Key: { tenant_id: { S: 'tenant1' }, hash_key: { S: 'evercookie#cookie123' } },
        })
        .resolves({ Item: undefined })
        .on(GetItemCommand, {
          Key: { tenant_id: { S: 'tenant1' }, hash_key: { S: 'stable#stable456' } },
        })
        .resolves({
          Item: marshall({
            tenant_id: 'tenant1',
  ...
  describe('queueProfileUpdate', () => {
    it('should send message to SQS profile queue', async () => {
      ...
      const call = calls[0];
      const input = call.args[0].input as { QueueUrl: string; MessageBody: string };
