...
import { ProfileService, ProfileServiceConfig, ProfileServiceDeps } from './profile-service';
...
describe('ProfileService', () => {
  ...
  describe('loadExistingProfile', () => {
    it('should return null when profile not found', async () => {
      ...
      const result = await service.loadExistingProfile('tenant1', 'dev_unknown');
  ...
  describe('buildTier1IndexEntries', () => {
    ...
    it('should return empty array when no hashes present', () => {
      ...
      const entries = service.buildTier1IndexEntries('tenant1', 'dev_123', fingerprint, ttl);
  ...
  describe('updateProfile', () => {
    it('should create new profile with defaults', async () => {
      ...
      await service.updateProfile('tenant1', 'dev_new', fingerprint, Date.now(), null);
    ...
    it('should preserve existing risk_score and flags', async () => {
      ...
      await service.updateProfile('tenant1', 'dev_existing', {}, Date.now(), existingProfile);
    ...
    it('should increment request_count', async () => {
      ...
      await service.updateProfile('tenant1', 'dev_123', {}, Date.now(), existingProfile);
  ...
  describe('updateTier1Indexes', () => {
    it('should write all index entries', async () => {
      ...
      const count = await service.updateTier1Indexes('tenant1', 'dev_123', fingerprint);
  ...
  describe('updateTier2Buckets', () => {
    it('should use UpdateItem with ADD for atomic bucket updates', async () => {
      ...
      const count = await service.updateTier2Buckets('tenant1', 'dev_123', fingerprint);
      ...
      const call = calls[0];
      expect(call.args[0].input.TableName).toBe(testConfig.tier2BucketsTable);
      expect(call.args[0].input.UpdateExpression).toContain('ADD device_ids');
    });

    it('should update multiple buckets in parallel', async () => {
      dynamoMock.on(UpdateItemCommand).resolves({});
