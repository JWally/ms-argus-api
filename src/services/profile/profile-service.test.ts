...
describe("ProfileService", () => {
  ...
  describe("updateSessionAnchorBucket / writeAnchor", () => {
    it("should return false when anchor key is null (missing fingerprint fields)", async () => {
      ...
      const result = await service.updateSessionAnchorBucket("dev_123", {});
      expect(result).toBe(false);
    });

    it("should return false for updateIpUaAnchorBucket when fields are missing", async () => {
      // buildIpUaAnchorKey returns null if ip_address or user_agent is missing
      const result = await service.updateIpUaAnchorBucket("dev_123", {});
      expect(result).toBe(false);
    });
  });
