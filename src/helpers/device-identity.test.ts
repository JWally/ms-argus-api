...
async function mintLegacyIdentity(h2Token: string): Promise<{
  pubkeyB64: string;
  sigB64: string;
}> {
...
/** Mint via the current payload-binding construction. */
async function mintCurrentIdentity(
  h2Token: string,
  stableHash: string,
  fuzzyHash: string,
...
describe("verifyDeviceIdentity", () => {
  ...
  it("returns probe_token_missing when no h2 token is in the payload", async () => {
    const { pubkeyB64, sigB64 } = await mintLegacyIdentity("any");
    const outcome = await verifyDeviceIdentity(
      basePayload({
  ...
  it("returns sig_invalid when sig is over the wrong bytes", async () => {
    const { pubkeyB64 } = await mintLegacyIdentity("intended-token");
    ...
    const { sigB64 } = await mintLegacyIdentity("different-token");
    const outcome = await verifyDeviceIdentity(
      basePayload({
        device_identity: { pubkey: pubkeyB64, sig: sigB64 },
  ...
  it("rejects a legacy XOR signature", async () => {
    const token = "abc123.99999999.deadbeef";
    const { pubkeyB64, sigB64 } = await mintLegacyIdentity(token);
    const outcome = await verifyDeviceIdentity(
      basePayload({
        device_identity: { pubkey: pubkeyB64, sig: sigB64 },
        sigintH2Token: token,
  ...
  // ── Current payload-binding contract ────────────────────────────────

  it("verifies a sig over (h2Token || stableHash || fuzzyHash)", async () => {
    const token = "v2tok.99999999.deadbeef";
    const { pubkeyB64, sigB64 } = await mintCurrentIdentity(
      token,
      "stable-a",
      "fuzzy-a",
    );
  ...
  it("rejects a signature replayed against a payload with different hashes", async () => {
  ...
  it("rejects a signature when stableHash differs", async () => {
    const token = "v2tok.99999999.deadbeef";
    const { pubkeyB64, sigB64 } = await mintCurrentIdentity(
      token,
      "stable-a",
  ...
  it("rejects a signature when fuzzyHash differs", async () => {
    const token = "v2tok.99999999.deadbeef";
    const { pubkeyB64, sigB64 } = await mintCurrentIdentity(
      token,
      "stable-a",
  ...
  it("rejects a legacy signature replayed across payloads", async () => {
