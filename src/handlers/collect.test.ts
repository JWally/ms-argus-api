...
vi.stubEnv('FINGERPRINT_TOPIC_ARN', 'arn:aws:sns:us-east-1:123456789:test-topic');
...
describe('collect handler', () => {
  ...
  describe('fingerprint collection', () => {
    it('should publish fingerprint to SNS and return success', async () => {
      ...
      // Need to parse body for the handler since middleware isn't running
      const event = createEvent();
      // @ts-expect-error - simulating parsed body
      ...
      expect(publishInput.TopicArn).toBe('arn:aws:sns:us-east-1:123456789:test-topic');
    ...
    it('should record success metric', async () => {
      ...
      expect(metrics.addMetric).toHaveBeenCalledWith('FingerprintCollected', 'Count', 1);
    });
  });
  ...
  describe('error handling', () => {
    ...
    it('should throw error when IP address is missing', async () => {
      ...
      // @ts-expect-error - simulating parsed body
      event.body = JSON.parse(event.body as string);
      // @ts-expect-error - remove sourceIp
    ...
    it('should throw error when headers are missing', async () => {
      ...
      // @ts-expect-error - simulating parsed body
      event.body = JSON.parse(event.body as string);
      // @ts-expect-error - remove headers
    ...
    it('should handle SNS publish failure', async () => {
      ...
      await expect(lambdaHandler(event)).rejects.toThrow();
      expect(metrics.addMetric).toHaveBeenCalledWith('FingerprintError', 'Count', 1);
    });
  });
});
