...
vi.stubEnv('PROFILE_QUEUE_URL', 'https://sqs.us-east-1.amazonaws.com/123456789/test-profile-queue');
...
describe('matching-worker handler', () => {
  ...
  const createSQSRecord = (body: object, messageId = 'test-msg-1'): SQSRecord => ({
    messageId,
  ...
  describe('error handling', () => {
    ...
    it('should return failed items only for records that fail', async () => {
      const successPayload = createFingerprintPayload({ session_id: 'success-session' });
      const failPayload = createFingerprintPayload({ session_id: 'fail-session' });
